...
FILE NAME:       sampleform.php 

FILE DESCRIPTION:
presents the editable details of a task record
...
file ssytem data write storage needed? (apache into group)
how much fule ssystem data storage? (with default)
archive approach 

db data storage needed? yes/no
how much (with default) 
archive approach

file group creation res+shortname
developers by modnet email address (to put them in the group)
with 7 permissions
...
//			IN A LATER SECTION, CODER WILL ADD ANY OF THEIR OWN VARIABLES IN THE SAME FORMAT AS THE STANDARD DESCRIBED HERE 
	...
	inputErrs = only relevant to and necessary in vars with an argType of 0 or 1 (both of which indicate that the value can be set by user-supplied arguments to the script. inputErrs holds a list of error codes (if any) for errors found in the user-supplied value. the value of inputErrs is set during data validation/verification 
	...
	*/
	...
	${$nameSpaceID."_var"}['outputMode']	= array("val"=> (function(){
		...
		if(isset($_REQUEST)=== true && array_key_exists('mode',$_REQUEST) === true){
			if($_REQUEST['mode'] === '_FUTURE'){
				//leaves space for a possible future where user interfaces are not HTML based.	
			}else{
				return "HTML";
			}
		}else{
	...
	})(), //end of self-invoking function to set output mode
	
	
	"argType" => -1, "mVal"=> false, "count" => 0, "mustPassOn" => false, "info" => "
	BELONGS TO THE TEMPLATE. it is assumed at the outset as default that this file should output an HTML page but the arguments received by the script might change that if HTML isnt the desired output. currently no alternative is accommodated but this
	variable is future-proofing
	");	
		
	${$nameSpaceID."_var"}['includeSecurity']  = array("val" => true, "argType" => -1, "mVal"=> false, "count" => 0, "inputErrs" => array(), "mustPassOn" => false, "info" => 
	...
	${$nameSpaceID."_var"}['supportedLocales']  = array("val"=> array("en-GB" => array("native" => "English (GB)","en" => "English (GB)"),"zh-Hans" => array("native" => "简体中文", "en" => "Chinese (Simplified)"),"en-Arab" => array("native" => "English (Arab)","en" => "English (Arab)")), "argType" => -1, "mVal"=> true, "count" => 3, "inputErrs" => array(), "mustPassOn" => false, "info" => 
	...
	${$nameSpaceID."_var"}['r2lLocaleScripts']  = array("val"=> array('Arab','Hebr','Nkoo','Syrc','Thaa','Tfng'), "argType" => -1, "mVal"=> true, "count" => 6 , "inputErrs" => array(), "mustPassOn" => false, "info" => 
	...
	${$nameSpaceID."_var"}['r2l'] =  array("val"=> false, "argType" => -1, "mVal"=> false, "count" => 0 , "inputErrs" => array(), "mustPassOn" => false, "info" => 
	...
	${$nameSpaceID."_var"}['invalidCharacters']  = array("val"=>  array("'", "\"", "<", ">", "/", "\\", ";", ":", "`", "{", "}"), "argType" => -1, "mVal"=> true, "count" => 11, "inputErrs" => array(), "mustPassOn" => false, "info" => 
	...
	${$nameSpaceID."_var"}['sr'] = array("val"=> "", "DIRTY" => "", "tmpVal" => "", "argType" => 0, "mVal"=> false, "count" => 1, "inputErrs" => array(), "mustPassOn" => true, "info" => "SR = server response. an error or message code number which is returned from the server and decoded into a message in the user's language");
	${$nameSpaceID."_var"}['srInt'] = array("val"=> "", "tmpVal" => "", "argType" => -1, "mVal"=> false, "count" => 1, "inputErrs" => array(), "mustPassOn" => true, "info" => "where the sr variable is found to hold a decimal fraction number eg 1.6, this holds the 1");
	${$nameSpaceID."_var"}['srDec'] = array("val"=> 0, "tmpVal" => "", "argType" => -1, "mVal"=> false, "count" => 1, "inputErrs" => array(), "mustPassOn" => true, "info" => "where the sr variable is found to hold a decimal fraction number eg 1.6, this holds the 6");
	${$nameSpaceID."_var"}['srTimeElapsed'] = array("val"=> 0, "tmpVal" => "", "argType" => -1, "mVal"=> false, "count" => 1, "inputErrs" => array(), "mustPassOn" => true, "info" => "where the sr varaible is found to hold a decimal fraction and is split at teh decimal point srInt.srDec and srDec contains a unixtimestamp, the number of seconds elapsed between that timestamp and now can be recorded here");

	${$nameSpaceID."_var"}['JSONErrors'] = array("val"=> array(), "DIRTY"=> "", "argType" => -1, "mVal"=> false, "count" => 0, "mustPassOn" => false, "info" => "
	...
	as the errors are now going to be moved into the 'inputErrors' part of each corresponding variable.
	but keeping for now
	");
	
	${$nameSpaceID."_var"}['error'] = array("val"=> "", "DIRTY"=> "", "argType" => 0, "mVal"=> false, "count" => 0, "mustPassOn" => false, "info" => "
	BELONGS TO THE TEMPLATE.  used as a flag for the script to say that an error has/has not been encountered. in non-complex scripts it might hold the error code/ message with no value (a blank string) indicating that no error has been encountered. Generally don't expect to return the content of this to the user (unless user is a debugging developer)... Generally, instead, each anticipated user-supplied argument will store within its { nameSpaceID.'_var'}['xxxxxx']['inputErrs'] array, a list of error codes intended to be returned to the user where the front-end code will de-code them into an error message in the appropriate human langauge.
	Where an internal code error e.g SQL error has been encountered, it will be written into this variable. 
	");
	${$nameSpaceID."_var"}['errorInt'] = array("val"=> 0, "tmpVal" => "", "argType" => -1, "mVal"=> false, "count" => 1, "inputErrs" => array(), "mustPassOn" => true, "info" => "where the error variable is found to hold a decimal fraction number eg 3.9, this holds the 3");
	${$nameSpaceID."_var"}['errorDec'] = array("val"=> 0, "tmpVal" => "", "argType" => -1, "mVal"=> false, "count" => 1, "inputErrs" => array(), "mustPassOn" => true, "info" => "where the error variable is found to hold a decimal fraction number eg 3.9 this holds the 9");
	
	
	
	
	${$nameSpaceID."_var"}['STDIN'] 	= array("val"=>  (function () { if(php_sapi_name() == "cli"){stream_set_blocking(STDIN, 0); $fh = fopen('php://stdin', 'r'); $read  = array($fh);  $write = NULL; $except = NULL;if(stream_select( $read, $write,$except, 0 ) === 1 ){return $fh;}else{return false;}}else{return false;}})(),"argType" => -1, "mVal"=> true, "count" => 0, "mustPassOn" => false, "info" => "
	...
	${$nameSpaceID."_var"}['PASSTHRU_DIRTY'] 		= array("val"=> array(), "argType" => -1, "mVal"=> true, "count" => 0, "mustPassOn" => true, "info" => "
	...
	${$nameSpaceID."_var"}['loc']  = array("val"=> "en-GB", "argType" => 0, "mVal"=> false, "count" => 0, "inputErrs" => array(), "mustPassOn" => false, "info" => 
	"BELONGS TO THE TEMPLATE.  a list of locales with the associated language presented once its local langauge and again in english (for the developers convenience). This is used
	...
	${$nameSpaceID."_var"}['piecesOfArgVNameValuePair']  = array("val"=> array(), "argType" => -1, "mVal"=> true, "count" => 0, "inputErrs" => array(), "mustPassOn" => false, "info" => 
	...
	${$nameSpaceID."_var"}['klogin_link'] =  array( "val" => array("db_conn_file" =>  $_SERVER['DOCUMENT_ROOT']."/klogin/klogin_database.php"), "argType" => -1, "mVal"=> true, "count" => 0, "inputErrs" => array(), "mustPassOn" => false, "info" => 
	...
	//thought: i could make this a global by not putting it into the namespace but otherwise keep the same structure to the variable.
	${$nameSpaceID."_var"}['appStore_link'] =  array( "val" => array("db_conn_file" =>  $_SERVER['DOCUMENT_ROOT']."/klogin/appStore_database.php"), "argType" => -1, "mVal"=> true, "count" => 0, "inputErrs" => array(), "mustPassOn" => false, "info" => 
	"BELONGS TO THE TEMPLATE.  the variable holds a value 'db_conn_file' set by the coder to point to a php file which establishes a connection
...
//CODER - REPLACE THE xxx on the next line with your appID	
	${$nameSpaceID."_var"}['appID'] = array("val"=> "98", "argType" => 0, "mVal"=> false, "count" => 1, "inputErrs" => array(), "mustPassOn" => false, "info" => "
		...
		if((${$nameSpaceID."_var"}['appStore_link']['val']['db_conn']) === false){
			...
			if(!${$nameSpaceID."_var"}['result']['val']  = mysqli_query(${$nameSpaceID."_var"}['appStore_link']['val']['db_conn'], ${$nameSpaceID."_var"}['sql']['val'])){
				...
				if(mysqli_num_rows(${$nameSpaceID."_var"}['result']['val'] ) != 1){
					$error = 'sql found no apps with appID "'.${$nameSpaceID."_var"}['appID']['val'].'" in the "'. ${$nameSpaceID."_var"}['loc']['val'].'" language appStore';
					$allErrors['error'][] = $error;
					if(${$nameSpaceID."_var"}['debugging']['val'] === true){
						echo "<H3>FAIL! failed to get information about the app from the database</H3>";
					...
					if(${$nameSpaceID."_var"}['debugging']['val'] === true){
						...
						var_dump(${$nameSpaceID."_var"}['result']['val']);
						echo "</pre></details><H3>SUCCESS! got information about the app from the database</H3></div>";
					}
...
/////yyyyyyyyyyyyyyyyyyyyyyy
	...
	${$nameSpaceID."_var"}['userLocalTimeZone'] = array("val"=> "Europe/London", "argType" => 0, "mVal"=> false, "count" => 1, "inputErrs" => array(), "mustPassOn" => false, "info" => "
	...
	example:
	${$nameSpaceID."_var"}['chocolate'] = array("val"=> "mars bar", "DIRTY" => "", "tmpVal" => "", "argType" => 1, "mVal"=> false, "count" => 1, "inputErrs" => array(), "mustPassOn" => true, "info" => "an example of a variable set in the template script.");
	...
	mustPassOn = boolean. if true, the variable's name-value pair will always be included in this scripts output eg. within the JSON object if the script's output is JSON or as a querystring name-value pair if this script finishes by redirecting to another URL.
	inputErrs = used with argTypes 0 and 1. a ¬ delimited list of error codes (if any) returned during templated data validation/verification 
...
//CODER ACTION - CODER TO DECLARE THEIR OWN VARIABLES BELOW HERE 
	...
	//${$nameSpaceID."_var"}['chocolate'] = array("val"=> "mars bar", "DIRTY" => "", "tmpVal" => "", "argType" => 1, "mVal"=> false, "count" => 1, "inputErrs" => array(), "mustPassOn" => true, "info" => "an example of a variable set in the template script.");
	${$nameSpaceID."_var"}['medName'] = array("val"=> "", "DIRTY" => "", "tmpVal" => "", "argType" => 0, "mVal"=> false, "count" => 1, "inputErrs" => array(), "mustPassOn" => true, "info" => "an example of a variable set in the template script.");
	${$nameSpaceID."_var"}['longDescription'] = array("val"=> "", "DIRTY" => "", "tmpVal" => "", "argType" => 0, "mVal"=> false, "count" => 1, "inputErrs" => array(), "mustPassOn" => true, "info" => "an example of a variable set in the template script.");
...
//END OF TEMPLATE	-GEOGRAPIC AND LOCALE SETTINGS (TIMEZONE/LANGUAGE ETC)	
	...
	if(${$nameSpaceID."_var"}['debugging']['val'] == true){
		...
		//if this is the top level document, not one document embedded inside another...
		if (str_replace('\\', '/', __FILE__) == $_SERVER['SCRIPT_FILENAME']) {
			header("Cache-Control: no-cache");
			header('X-Content-Type-Options: nosniff');  // to avoid IE sniffing (penetration testing 18/12/13)
			header("Expires: -1");
			date_default_timezone_set('Europe/London'); //server-side this is always Europe/London (GMT/UTC) but 
...
//TEMPLATE - CALL ANY INCLUDE FILES WHICH ARE USED FOR ALL FILES BUILT TO THE TEMPLATE STANDARD L@nce
	if(${$nameSpaceID."_var"}['debugging']['val'] === true){
	...
	include_once($_SERVER['DOCUMENT_ROOT'] ."/klogin/urltracker.php");
...
if(${$nameSpaceID."_var"}['debugging']['val']){
		print "<br>debugging = ".${$nameSpaceID."_var"}['debugging']['val']."<br>";
		print "<br>authenticated = ".$authenticated."<br>";
}
...
if (str_replace('\\', '/', __FILE__) == $_SERVER['SCRIPT_FILENAME']) { ?>
<!DOCTYPE html>
<html lang="<?php echo ${$nameSpaceID."_var"}['loc']['val'];?>" <?php if(${$nameSpaceID."_var"}['r2l']['val'] == true){echo 'dir="rtl"';}?>>
<head>
	<meta charset="utf-8">
	<meta content="IE=edge,chrome=1" http-equiv="X-UA-Compatible">
	<meta content="Default page" name="description">
	<meta content="width=device-width, initial-scale=1" name="viewport">
	<title><?php echo $$nameSpaceID['title']['innerHTML'];?></title>
	...
	<?php if(isset(${$nameSpaceID."_var"}['r2l']['val']) && ${$nameSpaceID."_var"}['r2l']['val']){ ?>
	<style>
	...
	if(isset($_REQUEST) === true){
		...
		foreach(array_keys($_REQUEST) as ${$nameSpaceID."_var"}['rkey']['val']){
			...
			if(isset(${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]) === true && ${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['argType'] >= 0){
				...
				${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['DIRTY'] = urldecode($_REQUEST[${$nameSpaceID."_var"}['rkey']['val']]);
				$e = -1;
				if(strpos(${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['DIRTY'], "¬")){  
					//check if the query string assserts that there are input errors with the field
					${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['DIRTY'] = explode("¬",${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['DIRTY']);
					
					
					
					${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['count'] = count(${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['DIRTY']);
					${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['inputErrs'] = array_slice(${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['DIRTY'],1);
							
					for($i=0;$i< count(${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['inputErrs']);$i++){
						if(isset($errors[${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['inputErrs'][$i]])){
							${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['inputErrs'][$i] = $errors[${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['inputErrs'][$i]]['msg'];
						}else{
							${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['inputErrs'][$i] = $errors[57]['msg'] . ' '. ${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['inputErrs'][$i];//unrecognized error code
						} 
						...
						${$nameSpaceID."_var"}['JSONErrors']['val'][${$nameSpaceID."_var"}['rkey']['val']][] = ${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['inputErrs'][$i];
						
					}

					
					//as the value for this field has been 'rejected' by the back end, i want to re-present the value but i am right to be suspicious about it so before accepting it out of the DIRTY form, strip any questionable characters from it
					${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['val'] = str_replace(${$nameSpaceID."_var"}['invalidCharacters']['val'], "",${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['DIRTY'][0]);
					if(${$nameSpaceID."_var"}['debugging']['val'] == true){
						echo ${$nameSpaceID."_var"}['rkey']['val']." ERROR******************************<BR><pre>";
						var_dump(${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['inputErrs']);
						echo "</pre></br>"; 
					}
						
	
				}else{ //if no input errors are being asserted from the query string
					...
					if(function_exists("check".${$nameSpaceID."_var"}['rkey']['val'])=== true){
						...
						if($e == 0){
							...
							if(${$nameSpaceID."_var"}['rkey']['val'] == 'sr'){
								...
								${$nameSpaceID."_var"}['srInt']['val'] = intval(${$nameSpaceID."_var"}['sr']['DIRTY']);			

								if((is_numeric(${$nameSpaceID."_var"}['srInt']['val']) && ${$nameSpaceID."_var"}['srInt']['val'] > 0)){
									if(isset($serverResponses[${$nameSpaceID."_var"}['srInt']['val']]['msg'])){
										${$nameSpaceID."_var"}['sr']['val'] = $serverResponses[${$nameSpaceID."_var"}['srInt']['val']]['msg'];
									}else{
										${$nameSpaceID."_var"}['sr']['val'] = $errors[59]['msg'] . ' ' . ${$nameSpaceID."_var"}['srInt']['val']; //"unrecognized response from server"
									}
									if(strpos(${$nameSpaceID."_var"}['sr']['DIRTY'],".") !== false){	
										${$nameSpaceID."_var"}['srDec']['val'] = substr((${$nameSpaceID."_var"}['sr']['DIRTY']),strlen(${$nameSpaceID."_var"}['srInt']['val'])+1);
										if(${$nameSpaceID."_var"}['srDec']['val']< time()){
											${$nameSpaceID."_var"}['srTimeElapsed']['val'] = strtotime('now') - strtotime('@'.${$nameSpaceID."_var"}['srDec']['val']);
										}
									}
								}else{
									${$nameSpaceID."_var"}['sr']['val'] = $errors[59]['msg']; //"unrecognized response from server"
								}
							}elseif(${$nameSpaceID."_var"}['rkey']['val'] == 'error'){
								
								if(${$nameSpaceID."_var"}['error']['val'] != ""){
									${$nameSpaceID."_var"}['errorInt']['val'] = intval(${$nameSpaceID."_var"}['error']['DIRTY']);				
									if((is_numeric(${$nameSpaceID."_var"}['errorInt']['val']) && ${$nameSpaceID."_var"}['errorInt']['val'] > 0)){
										if(isset($errors[${$nameSpaceID."_var"}['errorInt']['val']]['msg'])){
											${$nameSpaceID."_var"}['error']['val'] = $errors[${$nameSpaceID."_var"}['errorInt']['val']]['msg'];
										}else{
											${$nameSpaceID."_var"}['error']['val'] = $errors[59]['msg']; //"unrecognized response from server"
										}
									}else{
										${$nameSpaceID."_var"}['error']['val'] = $errors[59]['msg']; //"unrecognized response from server"
									}
							...
							if(${$nameSpaceID."_var"}['debugging']['val'] === true){
								echo "the value in argument '". ${$nameSpaceID."_var"}['rkey']['val']."' is considered valid because the function check" . ${$nameSpaceID."_var"}['rkey']['val'] . "() returned a response of or equivalent to 0 as an error code (meaning no errror).<BR>
							...
							//${$nameSpaceID."_var"}['error']['val']  = "invalid value in variable '".${$nameSpaceID."_var"}['rkey']['val']."'"; 
							
							${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['inputErrs'][] = $e; //$e holds the error code at this point, which is going to be returned to user.
							
							//shouldn't i also add the error to $JSONErrors ? yes
							if(in_array($e,$errors)){
								${$nameSpaceID."_var"}['JSONErrors']['val'][${$nameSpaceID."_var"}['rkey']['val']][] = $errors[$e]['msg'];
							}else{
								${$nameSpaceID."_var"}['JSONErrors']['val'][${$nameSpaceID."_var"}['rkey']['val']][] = $errors[57]['msg']." ".$e; //unrecognized error code
							}
							
							//as the value for this field has been 'rejected' here, i want to re-present the value but i am right to be suspicious about it so before accepting it out of the DIRTY form, strip any questionable characters from it
							${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['val'] = str_replace(${$nameSpaceID."_var"}['invalidCharacters']['val'], "",${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['DIRTY']);
							 
							
							if(${$nameSpaceID."_var"}['debugging']['val'] === true){
								echo ${$nameSpaceID."_var"}['error']['val']."<BR>";
								echo "this script anticipated receiving an argument called '".${$nameSpaceID."_var"}['rkey']['val']."' and received it. it attempted to verify and validate the value received in that variable. It used the check" . ${$nameSpaceID."_var"}['rkey']['val'] . "() function which is probably defined in '".$_SERVER['DOCUMENT_ROOT']."/".${$nameSpaceID."_var"}['appStore_link']['val']['documentRoot'] ."/fieldValidationFunctions.php'. The check" . ${$nameSpaceID."_var"}['rkey']['val'] . "() function exists, and received the value provided to it (value not shown here to avoid code injection). The function returned error code: " . $e. " meaning that the value which was checked is considered to be invalid. <BR>";
								
								
								//see if i can decode the error code here.
								//var_dump(("check".${$nameSpaceID."_var"}['rkey']['val'])(${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['rkey']['val']]['DIRTY']));
								echo "<BR>";
							}
						}
						
					}else{	// if checkxxx() function doesnt exist
						echo "<pre>";
					...
					} //end of if checkxxx() function exists
					
				}	//if there are input errors asserted by the query string
		...
		foreach(array_keys(${$nameSpaceID."_var"}) as ${$nameSpaceID."_var"}['vKey']['val']){
			if(${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['vKey']['val']]['argType'] === 1 && ${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['vKey']['val']]['val'] === ""){
				//set generic error flag so this script can quickly see an error has ocurred
				${$nameSpaceID."_var"}['error']['val'] = "missing mandatory value for argument ". ${$nameSpaceID."_var"}['vKey']['val'];
				//add the error info to the related variable's inputErrs array so that it is returned to the user
				${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['vKey']['val']]['inputErrs'][] = "60"; //mandatory value missing
				if(${$nameSpaceID."_var"}['debugging']['val'] === true){
					echo "missing a mandataory value for argument called ". ${$nameSpaceID."_var"}['vKey']['val']."<BR>";
				}
			}
		}
		if(${$nameSpaceID."_var"}['debugging']['val'] === true){
			if(strlen(${$nameSpaceID."_var"}['error']['val'])< 37 || substr(${$nameSpaceID."_var"}['error']['val'],0,37) !== "missing mandatory value for argument "){
				...
				echo "<H3>FAIL! One or more mandated arguments are not set. Either the 'CODER TO DECLARE THEIR OWN VARIABLES HERE' section of '" . __FILE__ ."' mistakenly identifies a non-mandatory argument as mandatory, else a mandatory argument was genuinely not set (or was unset by the code). Most likely this means that a averiable was expected in the \$_REQUEST array but was not present.</H3></div>";
		...
		foreach(array_keys(${$nameSpaceID."_var"}) as ${$nameSpaceID."_var"}['vKey']['val']){
			...
			if(${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['vKey']['val']]['argType'] >= 0 && ${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['vKey']['val']]['mVal'] == true && count(${$nameSpaceID."_var"}[${$nameSpaceID."_var"}['vKey']['val']]['inputErrs']) == 0){
...
if(${$nameSpaceID."_var"}['error']['val']== 0 || ${$nameSpaceID."_var"}['error']['val'] == ""){	?>
			<script>
				...
				var locale = '<?php echo ${$nameSpaceID."_var"}['loc']['val']; ?>';
				var xhr = [];
				var xhrCounter = 0;
	...
	<?php echo $nameSpaceID;?>.init = function(){
		...
		var countDownDate = new Date("Jun 10, 2023 3:00:00").getTime();
		...
		//template - set initial error reporting bubble for any errors passed in URL (which php has sumarized to JSONErrors js object)
		if(document.getElementById('<?php echo $nameSpaceID . "_form";?>')){
			...
			//console.log('<?php //var_dump(${$nameSpaceID."_var"}['JSONErrors']['val']);?>');
			
			
			
			var JSONErrors = JSON.parse('<?php echo json_encode(${$nameSpaceID."_var"}['JSONErrors']['val']); ?>');
			console.log(JSONErrors);
			...
			[].forEach.call(document.getElementById('<?php echo $nameSpaceID . "_form";?>').querySelectorAll('input, select, textarea'), (e)=>{
				...
				if(e.name in JSONErrors){
					//e.setCustomValidity(JSON.stringify(JSONErrors[e.name]));
				...
				e.addEventListener( "focus",  function(event){
						if((e.validationMessage != "" && e.validationMessage != " " && e.validationMessage != "&nbsp;") || e.dataset.infomsg != ""){
							console.log('form <?php echo $nameSpaceID . "_form";?> focus');
							inflateMsgBubble(e);
						}else{
							popMsgBubble(e);
						}
		...
		[].forEach.call(document.querySelectorAll('button.toolbarIconWrapper'), (e)=>{
			e.addEventListener("mouseover", function () {
				displayCustomErrorInfoMessage(this);					
			}, true );
		});
	...
	<?php echo $nameSpaceID;?>.selectCustomErrorMessage = function (that){
		...
		if(that.validity.badInput){
			...
			<?php echo $$nameSpaceID['selectCustomErrorMessageJS']['badInput'].PHP_EOL;?>
		}else if(that.validity.rangeOverflow){
			<?php echo $$nameSpaceID['selectCustomErrorMessageJS']['rangeOverflow'].PHP_EOL;?>
		}else if(that.validity.rangeUnderflow){
			<?php echo $$nameSpaceID['selectCustomErrorMessageJS']['rangeUnderflow'].PHP_EOL;?>
		}else if(that.validity.stepMismatch){
			<?php echo $$nameSpaceID['selectCustomErrorMessageJS']['stepMismatch'].PHP_EOL;?>
		}else if(that.validity.tooLong){
			<?php echo $$nameSpaceID['selectCustomErrorMessageJS']['tooLong'].PHP_EOL;?>
		}else if(that.validity.typeMismatch){
			<?php echo $$nameSpaceID['selectCustomErrorMessageJS']['typeMismatch'].PHP_EOL;?>
		}else if(that.validity.valueMissing){
			<?php echo $$nameSpaceID['selectCustomErrorMessageJS']['valueMissing'].PHP_EOL;?>
		}else if(that.validity.patternMismatch){
			<?php echo $$nameSpaceID['selectCustomErrorMessageJS']['patternMismatch'].PHP_EOL;?>
		}else if(that.validity.tooShort){
			<?php echo $$nameSpaceID['selectCustomErrorMessageJS']['tooShort'].PHP_EOL;?>
		}else{
		...
		displayCustomErrorInfoMessage(that);
			...
			<?php if (str_replace('\\', '/', __FILE__) == $_SERVER['SCRIPT_FILENAME']){?>
			</head>
			<body>
			...
			<?php }?>
			<form id="<?php echo $nameSpaceID . "_form";?>" accept-charset="utf-8" action="process_sampleform.php" name="<?php echo $nameSpaceID . "_form";?>" method="POST">
				<fieldset>
				<?php if(${$nameSpaceID."_var"}['debugging']['val'] && isset(${$nameSpaceID."_var"}['sr']['val']) && ${$nameSpaceID."_var"}['sr']['val'] != ""){
				echo '<div id="serverResponse">'.PHP_EOL;
				echo ${$nameSpaceID."_var"}['sr']['val'];
				echo '</div>'.PHP_EOL;
				}?>
				...
				<!--display should depend on if the values are set in querystring and if there are errors or not-->			
				<div class="inputRow">		
					<label id="medNameLabel" class="inputLargeLabel" for="medName"><?php echo $$nameSpaceID['medNameLabel']['innerHTML'];?></label>
					<div id="medNameTip" class="inputTip"><?php echo $$nameSpaceID['medNameTip']['innerHTML'];?></div>				
					<input 
					tabindex="1" 
					accesskey="<?php echo $$nameSpaceID['medName']['accesskey'];?>" 
					type="text"
					name="medName" 
					id="medName" 
					size="<?php echo $$nameSpaceID['medName']['maxlength'];?>" 
					title="" 
					class="form-control field <?php echo ${$nameSpaceID."_var"}['loc']['val'];?>"
					autocomplete ="off"
					placeholder="<?php echo $$nameSpaceID['medName']['placeholder'];?>"
					onfocus="this.reportValidity();"
					oninput="
					this.reportValidity();
					if(this.validity.valid){
						this.setCustomValidity('');
						document.getElementById(this.id +'Error').innerHTML ='&nbsp';
					}else{
						<?php echo $nameSpaceID;?>.selectCustomErrorMessage(this);
					}" 
					value="<?php echo ${$nameSpaceID."_var"}['medName']['val'];?>";
					required="required" 
					minlength="<?php echo $$nameSpaceID['medName']['minlength'];?>" 
					maxlength="<?php echo $$nameSpaceID['medName']['maxlength'];?>" 
					pattern="<?php echo $$nameSpaceID['medName']['pattern'];?>"
					data-infomsg="">
					<span class="inputErrorIcon" id="medNameErrorIcon">&nbsp;</span>
					<?php
					if(isset(${$nameSpaceID."_var"}['r2l']['val']) && ${$nameSpaceID."_var"}['r2l']['val'] == true){
						$direction = 'rtl';
					}else{
						$direction = 'ltr';
					}
					echo generateHTMLMsgBubble('medName',${$nameSpaceID."_var"}['medName']['inputErrs'],$direction);
					?>
					<div class="clearRow">&nbsp;</div>
					<div class="inputErrorMessage" id="medNameError"  ><?php if(count(${$nameSpaceID."_var"}['medName']['inputErrs']) > 0){echo implode(",", ${$nameSpaceID."_var"}['medName']['inputErrs']);}else{ echo '&nbsp;';}?></div>
				</div><!--end of inputRow-->
				<div class="clearRow">&nbsp;</div>
				the textarea control needs to have its DOMContent.php set

				<div class="inputRow">							
					<label id="longDescriptionLabel" class="inputLargeLabel" for="longDescription"><?php echo $$nameSpaceID['longDescriptionLabel']['innerHTML'];?></label>
					<div id="longDescriptionTip" class="inputTip"><?php echo $$nameSpaceID['longDescriptionTip']['innerHTML'];?></div>
					<textarea 
					tabindex="1" 
					accesskey="<?php echo $$nameSpaceID['longDescription']['accesskey'];?>" 
					type="text"
					name="longDescription" 
					id="longDescription" 
					size="<?php echo $$nameSpaceID['longDescription']['maxlength'];?>" 
					title="" 
					class="form-control field <?php echo ${$nameSpaceID."_var"}['loc']['val'];?>"
					autocomplete ="off"
					placeholder="<?php echo $$nameSpaceID['longDescription']['placeholder'];?>"
					onfocus="this.reportValidity();"
					rows="5"
					oninput="
					this.reportValidity();
					...
					required="required" 
					minlength="<?php echo $$nameSpaceID['longDescription']['minlength'];?>" 
					maxlength="<?php echo $$nameSpaceID['longDescription']['maxlength'];?>" 
					data-infomsg=""
					pattern="<?php echo $$nameSpaceID['longDescription']['pattern'];?>"
					><?php echo urldecode(${$nameSpaceID."_var"}['longDescription']['val']);?></textarea>
					<span class="inputErrorIcon" id="longDescriptionErrorIcon">&nbsp;</span>
					<?php
					if(isset(${$nameSpaceID."_var"}['r2l']['val']) && ${$nameSpaceID."_var"}['r2l']['val'] == true){
						$direction = 'rtl';
					}else{
					...
					echo generateHTMLMsgBubble('longDescription',${$nameSpaceID."_var"}['longDescription']['inputErrs'],$direction);?>
					<div class="clearRow">&nbsp;</div>
					<div class="inputErrorMessage" id="longDescriptionError" ><?php if(count(${$nameSpaceID."_var"}['longDescription']['inputErrs']) > 0){echo implode(",", ${$nameSpaceID."_var"}['longDescription']['inputErrs']);}else{ echo '&nbsp;';}?></div>
				</div><!--end of inputRow-->
				<div class="clearRow">&nbsp;</div>
				...
				<div class="buttonRow">
					<?php 		
					if (str_replace('\\', '/', __FILE__) == $_SERVER['SCRIPT_FILENAME']) { ?>
					<button tabindex="11" class="topWindowOnly" id="<?php echo $nameSpaceID . "_form";?>Cancel" accesskey="<?php echo $$nameSpaceID[$nameSpaceID.'_formCancel']['accesskey'];?>" type="button" value="Cancel" onclick="location.href='<?php echo $_SESSION['PREVIOUS_URLQS'];?>';" >
						<?php echo $$nameSpaceID[$nameSpaceID.'_formCancel']['innerHTML'];?>
					</button>
					
					<?php }?>
					<button tabindex="10"  class="disableIfFormIsInvalid"  id="<?php echo $nameSpaceID . "_form";?>Save" accesskey="<?php echo $$nameSpaceID[$nameSpaceID.'_formSave']['accesskey'];?>" type="submit" value="Update details" onclick="return <?php echo $nameSpaceID;?>.preSubmitCheck();">
						<?php echo $$nameSpaceID[$nameSpaceID.'_formSave']['innerHTML'];?>
					</button>
				</div>
				<div class="clearRow">&nbsp;</div>
				<div class="clearRow">&nbsp;</div>
			...
			<div id="modal-form-image" class="modal fade" data-backdrop="true" data-keyboard="true" data-focus="true">
			<div class="modal-dialog" style="max-width:calc(100vw - 3em)" >
				<div class="modal-content">
					<div id="modal-title-image"></div>
					<div class="modal-body">
						<div style="position:relative;height:100%;">		
							 <?php include( $_SERVER['DOCUMENT_ROOT'] ."/kloud/picsform.php");?>
						</div>
					</div>
				</div>
			</div>
...
<?php 
	        ...
	        echo "an error stopped the main content of this page from loading.";
	}
...
<script>
	...
	(else it would go in the functionLibrary.js file*/?>
		...
		if(${$nameSpaceID."_var"}['sr']['val'] != ""){	
			if(${$nameSpaceID."_var"}['srTimeElapsed']['val']  < 20){?> //if < 20 secs since server responded (or no time limit because elapsed = 0) 		
				$('#modalMsg').html('<?php echo addslashes(${$nameSpaceID."_var"}['sr']['val'] );?>');
		...
		if(${$nameSpaceID."_var"}['error']['val'] != ""){
			if(intval(${$nameSpaceID."_var"}['error']['val']) >0 && isset($errors) && isset($errors[${$nameSpaceID."_var"}['error']['val']]) && isset($errors[${$nameSpaceID."_var"}['error']['val']]['msg'])){$modalMsg = $errors[${$nameSpaceID."_var"}['error']['val']]['msg'];}else{$modalMsg = ${$nameSpaceID."_var"}['error']['val'];}?>
